...
const AboutPage = () => {
  ...
  useEffect(() => {
    ...
    let resizeTimer: NodeJS.Timeout | string | number | undefined;
    const handleResize = () => {
        ...
        <a href="#Portofolio">
          <div
            className={`grid grid-cols-1 md:grid-cols-$${
              statsData.length === 2 ? 2 : 3
            } gap-6 mt-16 cursor-pointer`}
          >
            {statsData.map((stat) => (
              <Card key={stat.label} {...stat} />
            ))}
          </div>
        </a>
      </div>
...
const Card = memo(
  ({ icon: Icon, color, value, label, description, animation }: CardProps) => (
    <div
      data-aos={animation}
      data-aos-duration={1300}
